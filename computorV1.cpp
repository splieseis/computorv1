...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/31 12:09:34 by spliesei          #+#    #+#             */
/*   Updated: 2019/06/20 16:16:49 by spliesei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::vector <double> getPolynom(std::string lhs, std::string rhs)
{
	...
	for (size_t num = 0; num < lhs_pos.size(); num++)
	{
		if ((isdigit(lhs[lhs_pos.at(num)]) || ((lhs[lhs_pos.at(num)] == '+' || lhs[lhs_pos.at(num)] == '-') && isdigit(lhs[lhs_pos.at(num) + 1]))) && lhs.substr(lhs_pos.at(num)).size() != 0)
			reducedPolynom.push_back(std::stod(lhs.substr(num)));
	}
	for (size_t num = 0; num < rhs_pos.size(); num++)
	{
		if (i < reducedPolynom.size() && rhs.substr(rhs_pos.at(num)).size() != 0)
			reducedPolynom.at(i) -= std::stod(rhs.substr(rhs_pos.at(num)));
		i++;
	}
  while (reducedPolynom.size() != 0 &&  reducedPolynom.at(reducedPolynom.size() - 1) == 0)
    reducedPolynom.pop_back();
...
void printReducedForm(std::vector <double> &polynom)
{
	...
	for (size_t num = 0; num < polynom.size(); num++)
	{
		polynom.at(num) >= 0.0 ? sign = '+' : sign = '-';
		if (i == 0)
    {
      if (polynom.at(num) < 0)
        reducedForm = reducedForm + sign + removeZeros(std::to_string(ft_abs(polynom.at(num)))) + " ";
      else
        reducedForm = reducedForm + removeZeros(std::to_string(ft_abs(polynom.at(num)))) + " ";
    }
		else if (i == 1)
			reducedForm = reducedForm + sign + " " + removeZeros(std::to_string(ft_abs(polynom.at(num)))) + " * X" + " ";
		else if (polynom.at(num) > 0)
			reducedForm = reducedForm + sign + " " + removeZeros(std::to_string(ft_abs(polynom.at(num)))) + " * X^" + std::to_string(i) + " ";
		else if (polynom.at(num) < 0)
			reducedForm = reducedForm + sign + " " + removeZeros(std::to_string(ft_abs(polynom.at(num)) * 1)) + " * X^" + std::to_string(i) + " ";
		i++;
	}
...
int main()
{
    ...
    if (input.size() != 0)
    {
      ...
      // std::string polynom_str  = "37 * X^0 + 12 * X^1 + 1 * X^2 = 0 * X^0";
  		// std::string polynom_str = "1 * X^0 + 0 * X^1 + 0 * X^2 = 0";
